...
func (g *schemaGenerator) javaType(t reflect.Type) string {
	...
	if t.Kind() == reflect.Struct && ok {
		...
		default:
			...
			if t.Name() == "LabelSelector" {
				return "io.fabric8.kubernetes.api.model.LabelSelector"
			}
			if t.Name() == "GroupKind" {
				return "me.snowdrop.applicationcrd.api.model.GroupKind"
			}
...
func (g *schemaGenerator) javaExtends(t reflect.Type) *JavaExtendsDescriptor {
	if _, ok := t.FieldByName("TypeMeta"); ok {
		return &JavaExtendsDescriptor{
			Reference: "#/definitions/github_com_kubernetes_sigs_application_vendor_k8s_io_apimachinery_pkg_apis_meta_v1_TypeMeta",
...
func (g *schemaGenerator) generate(t reflect.Type) (*JSONSchema, error) {
	...
	s := JSONSchema{
		ID:     "http://snowdrop.me/application-crd/v1/" + t.Name() + "#",
		...
		if field.Anonymous && field.Type.Kind() == reflect.Struct && (len(name) == 0 || strings.HasPrefix(name, "Common") || name == "PlanReference" || name == "TypeMeta") {
			...
			for k, v := range newProps {
				...
				case "apiVersion":
					...
					//Tune version for application crd
					apiGroup := filepath.Base(strings.TrimSuffix(path, apiVersion))
					pkgDesc, ok := g.packages[path]

					if apiVersion == "applicationcrd/v1beta1" {
						apiVersion = "applicationcrd.k8s.io/v1beta1"
					}
...
func pkgPath(t reflect.Type) string {
	return strings.TrimPrefix(t.PkgPath(), "github.com/snowdrop/application-crd-client/generator/vendor/")
}
